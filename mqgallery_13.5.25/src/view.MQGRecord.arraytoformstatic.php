...
$objForm = new MQGInputForm(332);
$objForm->setUrl($target);
$arrFields = array();
//Prepare: create the form fields
foreach ($fields as $name=>$params)  {   
  if (isset($params['active']) && '0' == $params['active'])  continue;
  if ('output' == $params['type'])  continue;
  
  // Daten aus Db verfügbar
  if (isset($params['column'])) {
    $value = self::${$params['column']};
    if (isset($params['displayas'])):
       if(is_array($params['type'])):
         foreach ($params['type'] as $key=>$type) {
           if (isset($value[$key])):
             $value[$key] = self::transform($value[$key],$params['displayas']);
           else:
             $value[$key] = '';
           endif;
         }
       else:
         $value=self::transform($value,$params['displayas']);
       endif;
    endif;
  }else{
    $value=NULL;   
    ...
    if ('output' == $params['type']) {
      ...
      foreach ($params['type'] as $key=>$type) {
        ...
        if (isset($params['saveas'])):
          $value[$key] = self::transform($value[$key],$params['saveas']);
        endif;
      }
    }else{
      //Single field
      ...
      if (isset($params['saveas'])):
        $value = self::transform($value,$params['saveas']);
      endif;
    }
